...
async function loadItems() {
  try {
    toggleUIState(true);
    const snapshot = await db.collection('sodexo').get();
    stateVersion.value++;
...
async function addNewItem() {
  ...
  try {
    ...
    const doc = await db.collection('sodexo').doc(name).get();
    ...
    await db.collection('sodexo').doc(name).set({ 
      link, 
      category,
      order: lastOrder + 1
    });
...
async function handleDelete(item) {
  ...
  try {
    ...
    await db.collection('sodexo').doc(item.name).delete();
...
async function moveItem(item, direction) {
  ...
  try {
    ...
    if (newIndex >= 0 && newIndex < categoryItems.length) {
      ...
      batch.update(db.collection('sodexo').doc(categoryItems[currentIndex].id), {
        order: categoryItems[currentIndex].order
      });
      batch.update(db.collection('sodexo').doc(categoryItems[newIndex].id), {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (newName !== item.name) {
        const doc = await db.collection('sodexo').doc(newName).get();
      ...
      // Operação Firestore
      const batch = db.batch();
      batch.delete(db.collection('sodexo').doc(item.name));
      batch.set(db.collection('sodexo').doc(newName), { 
        link: newLink, 
        category: newCategory 
      });
      await batch.commit();
